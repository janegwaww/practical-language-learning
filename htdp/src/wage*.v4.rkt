...
(define-struct name-amount [name ID amount])

; Low -> List-of-numbers
; computes the weekly wages for all weekly work records 
 
(check-expect
  (wage*.v3 (cons (make-work "Robby" "123" 11.95 39) '()))
  (cons (make-name-amount "Robby" "123" (* 11.95 39)) '()))
...
(define (wage.v3 w)
  (make-name-amount (work-employee w) (work-ID w) (* (work-rate w) (work-hours w))))