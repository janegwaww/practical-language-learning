...
removeAt n xs | n > length xs = (Nothing, xs)
              | otherwise = (Just (single n xs), (remove 0 xs))
              where
                single :: Int -> [Int] -> Int
                single 0 (x:ys) = x
                single n (x:ys) = single (n-1) ys
                remove :: Int -> [Int] -> [Int]
                remove _ [] = []
                remove m (x:ys) | m == n = ys
                                | otherwise = x:remove (m+1) ys
...
evenLists :: [[Int]] -> [[Int]]
evenLists [] = []
evenLists (x:xs) | mod (length x) 2 == 0 = x:evenLists xs
...
sorted :: Int -> [Int] -> Bool
sorted _ [] = True
sorted n (x:xs) | (<) n x = sorted x xs
                  | otherwise = False
isIncreasing :: [Int] -> Bool
isIncreasing (x:xs) = sorted x xs
...
merge :: [Int] -> [Int] -> [Int]
merge [] ys = ys
merge (x:xs) ys = merge xs ((little x ys)++[x]++(greate x ys))
  where
    greate :: Int -> [Int] -> [Int]
    greate _ [] = []
    greate n (l:ls) | (<) n l = l:greate n ls
                    | otherwise = greate n ls
    little :: Int -> [Int] -> [Int]
    little _ [] = []
    little n (l:ls) | (>) n l = l:little n ls
                    | otherwise = little n ls
...
eval xs (Var v) = let may = lookup v xs in
                    case may of {
                      Just n -> Right n;
                      Nothing -> Left (VariableNotFound "err")}
eval ys (Add x y) =  eval ys (exec x y)
  where
    exec :: Expr -> Expr -> Expr
    exec i (Lit n) = let num = eval ys i in
                       case num of {
                         Right nn -> (Lit (n+nn));
                         Left _ -> (Lit n)}
    exec i (Var vi) = let num = eval ys i; head = eval ys (Var vi) in
                       case num of {
                         Right nn -> case head of {
                             Right nm -> (Lit (nm+nn));
                             Left _ -> (Var vi)}}
    exec i (Add n m) = let num = eval ys i; head = eval ys n in
                         case num of {
                                     Right nn -> case head of {
                                         Right nm -> (Add (Lit (nm+nn)) m);
                                         Left _ -> (Add n m)};
                                     Left _ -> (Add n m)}
...
constantFolding (Add x y) = exFold x 0 (exFold y (toInt x) (Lit 0))
exFold :: Expr -> Int -> Expr -> Expr
exFold (Lit x) n e = (Add (Lit (x+n)) e)
exFold (Var x) _ e = (Add (Var x) e)
exFold (Add x y) n e = case x of {
  (Lit x') -> exFold y (n+x') e;
  (Var y') -> exFold y n (Add (Var y') e)}
  where
    num = n + (toInt x)
toInt :: Expr -> Int
toInt (Lit x) = x
toInt (Var _) = 0
toInt (Add x y) = case x of {
  (Lit x') -> x' + (toInt y);
  (Var _) -> (toInt y)}
