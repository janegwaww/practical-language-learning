...
next w = toEnum ((fromEnum w + 1) `mod` 7)
...
daysTo w1 w2 | w1 == w2 = 0
             | otherwise = 1 + daysTo (next w1) w2
...
instance Semigroup Reward where
  (Reward n m) <> (Reward i j) = Reward (n<>i) (m||j)
...
instance Semigroup (List1 a) where
  (List1 n m) <> (List1 _ j) = List1 n (m<>j)

{- | Does 'List1' have the 'Monoid' instance? If no then why?

instance Monoid (List1 a) where
because it is not empty element
-}
...
--   foldMap f  Monday= mempty
-- instance Foldable Gold where
--   foldMap f (Gold 0) = (Gold 0)
-- instance Foldable Reward where
instance Foldable List1 where
  foldMap f (List1 n []) = f n
...
--   fmap f Sunday = Sunday
-- instance Functor Gold where
--   fmap f unGold = f unGold
