...
-}
strSum :: String -> Int
strSum str = sumOf 0 (words str)
  where
    sumOf :: Int -> [String] -> Int
    sumOf acc list =
      if null list
      then acc
      else sumOf (acc + (read (head list)::Int)) (tail list)
...
lowerAndGreater n list = compareEle 0 0 list
  where
    compareEle :: Int -> Int -> [Int] -> String
    compareEle g l li =
      if null li
      then (show n ++ " is greater than " ++show l ++" elements and lower than " ++show g ++" elements")
      else if n > (head li)
      then compareEle g (l+1) (tail li)
      else compareEle (g+1) l (tail li)
